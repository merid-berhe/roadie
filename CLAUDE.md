...
#### 2026-06-11 — 360° orbit camera + sunglasses & afro ✅
- **Completed:** masterRule bumped to v4.8. Camera now does a slow continuous orbit (48s/rev): low at front/back, rising to an aerial beat at the sides (roadside corridor too narrow for a low side shot — city facades clamped ≥4.2 from centerline, pagodas 5.6+, desert poles 4.1 so the orbit path stays clear). World made symmetric for the orbit: props in a ±117 z-band with scale-fade at the edges (groups only — `MovingEntity.fade`), 3 terrain tiles (coverage ±220), road/rail/sidewalk/ocean statics span ±230, mirrored backdrop silhouettes + clouds at both horizons. **Found the car had been driving rear-first since the camera flip** — the vertical-lamp end everyone (including the user's reference screenshot) took for the front is the rear; the round-headlight grille end is the nose (model-local −x, LHD). Flipped, occupants moved into the actual front seats (they'd been in the back row, hidden behind seat backs), driver (man) now behind the wheel viewer-right. Characters: black afro for the man (crown kept clear of the face — first attempt engulfed it), sunglasses on both (also solves the featureless-face problem face-forward).
- **Verified:** typecheck / 4/4 / build green. Screenshots: front hero shot with both faces+shades readable through windshield (cropped close-up), front-quarter, aerial side, rear; desert + coast + mountain.
- **Next:** user review. Watch on real play: orbit pacing (48s), city lamp near-passes at side angles, prop fade-in visibility at band edges in clear-fog themes.

---

#### 2026-06-10 (night) — Locked exterior camera: the car becomes the hero ✅
- **Completed:** masterRule bumped to v4.7 — back-seat POV replaced by a **locked, slightly elevated exterior camera ahead of the car**, looking at its front (user-directed). `PlayCanvasRideScene`: cicada_flat.glb loaded into the player path (513 KB container asset, holder entity carries yaw −90°/scale so the GLB root keeps its orientation fix — model forward is +x and setting euler directly tipped the car), scaled to track width, wheels seated on the road; man + woman occupants rotated to face camera, seated in the cabin (driver = viewer-left, the model is RHD); world motion reversed (`base − offset`, `WRAP_AHEAD=10` so props recycle behind the camera); terrain prop height sampling now interpolates across columns (boulders floated on steep alpine slopes); fake contact-shadow ellipse; camera aspect-aware (hero framing landscape, pulled back portrait); cockpit overlay (roof/pillars/dash/mirror) deleted; car bob/roll replaces camera sway. Gesture contract preserved — `Riding.tsx` still untouched.
- **Verified:** typecheck / tests 4/4 / build green. Screenshot-judged: all 4 themes landscape, desert portrait, coast wave gesture, late positions. 4 iterations (flipped car → side-facing car → giant shadow blob → floating boulders → fixed).
- **Next:** user review in browser. Possible polish: occupant heads slightly visible through rear window?, wheel rotation is invisible (dark discs) so skipped, mood-tint integration still open.
