...
- **Next:** M2 — lobby composition (shared mood seed + Driver/Passenger choices, each sees the other's land, both `ready` advances phase). Add §5 recipe types + `buildPrompt()` to `/shared`. Accept: combined `recipe` correct server-side.
