# Roadie — Build Spec v4.8 (Production)
...
**Platform:** browser-first responsive web (decision 2026-06-10: the **desktop browser is the primary play surface**; the game must still work in mobile Safari and Chrome, but desktop is no longer "a bonus" — frame compositions, asset budgets, and UI density for desktop first, then verify mobile).
...
**Engine + art direction (current default ride renderer): PlayCanvas, flat-shaded low-poly.** Decision update (2026-06-10): the PlayCanvas renderer is no longer a bare engine spike — it now carries a committed art direction: **flat-shaded low-poly** (Alto's-Odyssey-adjacent), built 100% procedurally (no GLB assets in the player path). Per theme: gradient sky dome (vertex-colored, unlit/unfogged), sun/moon disc with theme-tuned additive glow, stars (desert/city), drifting cloud clusters, faceted vertex-colored terrain (two leapfrogging seamless tiles; per-theme height profiles — dunes, Big Sur cliff drop to ocean, alpine berms with rock/snow bands, flat city), static far-silhouette backdrops (mesa/ridge/peaks/skyline), and composed multi-part props (multi-arm saguaros + strata mesas + telephone poles; wind-swept Monterey cypress + guardrail + sailboats + whitecaps; tiered snow-capped pines + peaks; window-lit buildings + pagodas + torii gates + glowing street lamps). The pair rides in the flattened Cicada hero car (see v4.6/v4.7), occupants visible through the windshield, gesture markers, firework particles. Visual quality is verified via headless screenshots of `?scene=1&road=<theme>&t=<sec>` — every future scene change must be judged from rendered frames, not code.
...
### Camera & composition (slow 360° orbit)
**Decision 2026-06-11 (v4.8), refines v4.7:** the exterior camera **orbits the car continuously — one slow revolution every ~48s** (≈2.5 orbits per 2-min ride). Low cinematic framing at the front (faces through the windshield: this is when co-presence reads) and back, rising to a brief aerial beat at the sides (the corridor beside the road is too narrow for a low side shot — roadside props clear the orbit path by design: city facades ≥ 4.2 from centerline, desert poles at 4.1). The world is **symmetric around the car** so the orbit never sees it end: props live in a ±117 band with scale-fade at the edges, terrain runs 3 leapfrogging tiles (±220), road/ocean/sidewalk/rail statics span ±230, backdrop silhouettes and clouds exist at both horizons. The camera always looks at the car; the car carries the life (bob/roll). Rationale (user): a permanently front-locked camera begs "how are we seeing this?"; the orbit answers it cinematically and shows the whole destination world.
The hero car is the flattened Cicada (nose = the round-headlight end, model-local −x, LHD). Occupants: man (driver, black afro) + woman (long brown hair), **both in sunglasses** — face-forward visibility made featureless faces unacceptable; sunglasses add character while preserving facelessness/anonymity.
...
| v4.8 | 2026-06-11 | **Slow 360° orbit camera (48s/rev) replaces the locked front view; world made symmetric around the car** (±117 prop band w/ edge fade, 3 terrain tiles, mirrored backdrops/clouds, full-span statics). Car flipped nose-first (it had been driving rear-first since v4.7 — the model's vertical-lamp end is the rear). Occupants: sunglasses on both, black afro on the driver, seated in the actual front seats (LHD). | User: front-locked camera was implausible ("how can players see faces?"); orbit shows faces at the front beat and the whole world the rest of the time. Sunglasses solve the featureless-face problem while keeping anonymity. |
| v4.7 | 2026-06-10 | **Camera: locked exterior front view replaces back-seat POV.** Hero car (cicada_flat.glb) in the player ride path, occupants seated inside facing the camera, world motion reversed (streams past the car into the distance), fake contact shadow, aspect-aware camera distance. Procedural cockpit overlay removed. | The inside view kept fighting the framing ("ticky"); exterior elevated front view keeps the car + both riders always visible — same co-presence portrait, externalized. User-directed. |
| v4.6 | 2026-06-10 | **Platform priority flip: desktop browser is the primary play surface**; mobile Safari/Chrome remain supported, no longer the design target. | User decision. Affects framing (landscape first), asset budgets, and UI density. |
